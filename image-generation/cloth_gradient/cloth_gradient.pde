...
public void createTexture(DwSoftGrid2D body) {

  int nodex_x = body.nodes_x;
  int nodes_y = body.nodes_y;
  float nodes_r = body.nodes_r;

  int tex_w = Math.round((nodex_x-1) *nodes_r*2);
  int tex_h = Math.round((nodes_y-1) *nodes_r*2);

  PGraphics2D texture = body.texture_XYp;
...
#000000
  ...
  for (int i = 0; i <tex_h; i+=ystep) {
    for (int j = 0; j<tex_w; j+=xstep) {
      ...
      texture.fill(tweenColor);
      texture.noStroke();
      texture.rect(j, i, xstep, ystep);
