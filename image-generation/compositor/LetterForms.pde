PGraphics drawWord(String aWord, float ts, int maxW, int maxH) {
  PGraphics surface = createGraphics(maxW, maxH);

  /*
    layout letters in order,
    A  B  C  D
    E  F  G  H
    I  J  K  L
    M  N  O  P
  ...
  for (int i = 0; i < aWord.length(); i++){
    ...
    LetterForm lf = getLetter(c, ts, 2, color(200, 0, 127));
    lf.draw();

    surface.image(
      lf.surface, random(maxW), random(maxH)
    );
  }
...
// letter, timestamp, weight, color
LetterForm getLetter(char l, float ts, int w, color c) {
  switch(l){
