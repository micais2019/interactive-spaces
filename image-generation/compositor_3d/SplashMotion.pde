...
class SplashMotion {
  ...
  PShape create(int points, PApplet app) {
    ...
    while (vitr.hasNext()) {
      vitr.next().setColor(color(random(255), random(80), random(80, 180)));    //color, random
    }

    PShape splashmesh = WB_PShapeFactory.createSmoothPShape(mesh, app);
