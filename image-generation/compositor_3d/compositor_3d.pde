...
void setup() {
  ...
  // println(coverWidth * dpi, coverHeight * dpi);
  ...
  cloth = new ClothShape(width/2, floor(height * 3), 1200, 100);
...
void drawCloth(long ts) {
  ...
  if (CONTROL_POSITION) {
    translate(width/2 + width/4, 100, -400);
    rotateY(mouseX * 1.0f/width * TWO_PI);
    rotateX(mouseY * 1.0f/height * TWO_PI);
  } else {
    translate(width/2 + width/4, 100, -400);
    rotateY(2.670354); // mouseX * 1.0f/width * TWO_PI);
    rotateX(3.0089478); // mouseY * 1.0f/height * TWO_PI);
  }
